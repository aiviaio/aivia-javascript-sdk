const { isAddress, isString, isInteger } = require("./helpers/errorHandler");
...
function init(ENTRY_POINT, HTTP_PROVIDER, DEFAULT_GAS_PRICE) {
  isString(HTTP_PROVIDER, "HTTP_PROVIDER", "'HTTP_PROVIDER' must be a string URL");
  ...
  if (DEFAULT_GAS_PRICE) {
    isInteger(
      DEFAULT_GAS_PRICE,
      "DEFAULT_GAS_PRICE",
      "'DEFAULT_GAS_PRICE' must be a number(in wei)"
    );
  }
